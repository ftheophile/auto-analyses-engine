...
class Lsmom:
    ...
    def run_simulation(self, historical_data, startCapital):
        ...
        # portfolio_df.to_excel("./Data/lsmom-"+ fileName +".xlsx", sheetName)
        try:
            with pd.ExcelWriter("./Data/lsmom-"+fileName+".xlsx", mode='a') as writer:  # doctest: +SKIP
                portfolio_df.to_excel(writer, sheet_name=fileName+"--"+sheetName)
        except FileNotFoundError:
            portfolio_df.to_excel("./Data/lsmom-"+ fileName +".xlsx", sheetName)
        except:
            print("Lsmom could not write portfolio details to file")
