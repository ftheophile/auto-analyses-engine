...
def get_strat_scaler(portfolio_df, lookback, vol_target, idx, default):
    ...
    if len(capital_ret_history) == lookback: # enough data, the increase or decrease the scalar based on empirical vol of the strategy equity curve relative to target vol.
        annualized_vol = capital_ret_history.std() * np.sqrt(253) # annualized the vol from daily vol
        scalar_hist_avg = np.mean(strat_scaler_history)
        strat_scalar = scalar_hist_avg * vol_target / annualized_vol
        return strat_scalar
    else: # not enough data, just return a default value
        return default